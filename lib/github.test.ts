...
describe('fetchGitHubContributions', () => {
  ...
  it('uses GITHUB_TOKEN when GITHUB_PAT is not configured', async () => {
    delete process.env.GITHUB_PAT;
    process.env.GITHUB_TOKEN = 'ghp_actionsTokenAAAAAAAAAAAAAAAAAAAAAAAAA';
    vi.mocked(fetch).mockResolvedValue(
      mockResponse({
        data: {
          user: {
    ...
    expect(options?.headers).toMatchObject({
      Authorization: 'bearer ghp_actionsTokenAAAAAAAAAAAAAAAAAAAAAAAAA',
    });
  });

  it('verifies Authorization header uses GITHUB_TOKEN value in fallback path', async () => {
    delete process.env.GITHUB_PAT;
    process.env.GITHUB_TOKEN = 'ghp_myActionsTokenAAAAAAAAAAAAAAAAAAAAAAAAA';
    vi.mocked(fetch).mockResolvedValue(
      mockResponse({
        data: {
          user: {
    ...
    const [, options] = vi.mocked(fetch).mock.calls[0];
    expect(options?.headers).toMatchObject({
      Authorization: 'bearer ghp_myActionsTokenAAAAAAAAAAAAAAAAAAAAAAAAA',
    });
  });
